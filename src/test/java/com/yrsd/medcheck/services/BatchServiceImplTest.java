...
class BatchServiceImplTest {
    ...
    @Mock Organisations organisations;

    @InjectMocks
    private BatchServiceImpl batchService;


    private UserAccount manufacturerEmployee;
    private UserAccount wholesalerEmployee;
    private UserAccount retailerEmployee;
    private Organisation manufacture;
    private Organisation wholesaler;
    private Organisation retailer;
    private Drug drug;
    private Batch batch;
    private Pack pack;
    ...
    @BeforeEach
    void setUp() {
        manufacture = new Organisation();
        manufacture.setName("Manufacturer");
        manufacture.setOrganisationType(OrganisationType.MANUFACTURE);
        manufacture.setId("OMO");


        retailer = new Organisation();
        retailer.setId("retailer0");
        retailer.setName("Folyma");
        retailer.setOrganisationType(OrganisationType.RETAIL);


        wholesaler = new Organisation();
        wholesaler.setId("wholesaler0");
        wholesaler.setName("Wholesaler");
        wholesaler.setOrganisationType(OrganisationType.WHOLESALE);


        manufacturerEmployee = new UserAccount();
        manufacturerEmployee.setId("user-1-uuid");
        manufacturerEmployee.setUsername("Pfizer-NG");
        manufacturerEmployee.setRole(Role.MANUFACTURING_EMPLOYEE);
        manufacturerEmployee.setOrganisation(manufacture);

        wholesalerEmployee = new UserAccount();
        wholesalerEmployee.setId("user-2-uuid");
        wholesalerEmployee.setUsername("Big-Pharma-Dist");
        wholesalerEmployee.setRole(Role.WHOLESALE_EMPLOYEE);
        wholesalerEmployee.setOrganisation(wholesaler);

        retailerEmployee = new UserAccount();
        retailerEmployee.setId("user-3-uuid");
        retailerEmployee.setUsername("Olatunde-Pharmacy");
        retailerEmployee.setRole(Role.RETAIL_EMPLOYEE);
        retailerEmployee.setOrganisation(retailer);

        retailer.addUserAccount(retailerEmployee);
        wholesaler.addUserAccount(wholesalerEmployee);
        manufacture.addUserAccount(manufacturerEmployee);
        ...
        drug.setManufacturer(manufacturerEmployee.getOrganisation());
    ...
    void createBatch_Successful_ShouldCreateHierarchyAndReturnResponse() {
        ...
        request.setManufacturing_employee_Id(manufacturerEmployee.getUsername());
        ...
        when(userAccounts.findByUsername("Pfizer-NG")).thenReturn(Optional.of(manufacturerEmployee));
    ...
    void createBatch_UnauthorizedManufacturer_ShouldThrowException() {
        UserAccount fakeManufactureEmployee = new UserAccount();
        fakeManufactureEmployee.setId("FAKE01");
        CreateBatchRequest request = new CreateBatchRequest();
        request.setDrugId(drug.getId());
        request.setManufacturing_employee_Id(fakeManufactureEmployee.getId());

        when(userAccounts.findByUsername("FAKE01")).thenReturn(Optional.of(fakeManufactureEmployee));
        when(drugs.findById(drug.getId())).thenReturn(Optional.of(drug));
    ...
    void transferBatch_ManufacturerToWholesaler_Success() {
        TransferBatchRequest request = new TransferBatchRequest("batch-uuid", "user-1-uuid",
                "user-2-uuid");

        when(batches.findById("batch-uuid")).thenReturn(Optional.of(batch));
        when(userAccounts.findById("user-2-uuid")).thenReturn(Optional.of(manufacturerEmployee));

        when(organisations.findById("user-1-uuid")).thenReturn(Optional.of(wholesalerEmployee.getOrganisation()));
    ...
    void transferBatch_Manufacturer_AlreadyMoved_Fail() {
        TransferBatchRequest request = new TransferBatchRequest("batch-uuid", "user-2-uuid", "user-1-uuid");

        when(batches.findById("batch-uuid")).thenReturn(Optional.of(batch));
        when(userAccounts.findById("user-1-uuid")).thenReturn(Optional.of(manufacturerEmployee));
        when(organisations.findById("user-2-uuid")).thenReturn(Optional.of(wholesaler));
    ...
    void transferBatch_WholesalerToRetailer_Success() {
        TransferBatchRequest request = new TransferBatchRequest("batch-uuid", "user-3-uuid", "user-2-uuid");

        when(batches.findById("batch-uuid")).thenReturn(Optional.of(batch));
        when(userAccounts.findById("user-2-uuid")).thenReturn(Optional.of(wholesalerEmployee));
        when(organisations.findById("user-3-uuid")).thenReturn(Optional.of(retailer));

        BatchLogistics lastMove = new BatchLogistics();
        lastMove.setSender(manufacturerEmployee.getOrganisation());
        lastMove.setRecipient(wholesalerEmployee.getOrganisation());
    ...
    void transferBatch_Wholesaler_CustodyFail() {
        ...
        BatchLogistics lastMove = new BatchLogistics();
        lastMove.setRecipient(retailerEmployee.getOrganisation());
        when(batchLogisticsRepo.findTopByBatchOrderByCreatedDesc(batch)).thenReturn(Optional.of(lastMove));
    ...
    void sellPack_Success() {
        ...
        request.setRetailerId(retailerEmployee.getId());

        when(packs.findById(pack.getId())).thenReturn(Optional.of(pack));
        when(userAccounts.findById(retailerEmployee.getId())).thenReturn(Optional.of(retailerEmployee));
        ...
        batchMove.setRecipient(retailerEmployee.getOrganisation());
    ...
    void sellPack_Fail_NoCustody() {
        ...
        BatchLogistics batchMove = new BatchLogistics();
        batchMove.setRecipient(wholesalerEmployee.getOrganisation());
        when(batchLogisticsRepo.findTopByBatchOrderByCreatedDesc(batch)).thenReturn(Optional.of(batchMove));
